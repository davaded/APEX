...
export function TweetCard({ tweet, onClick }: TweetCardProps) {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-[#121212] rounded-2xl overflow-hidden cursor-pointer shadow-lg"
            style={{
        ...
        >
            ...
            <div className="flex items-start justify-between p-5 pb-3">
                <div className="flex items-center gap-3">
                    <div className="relative w-8 h-8 rounded-full overflow-hidden bg-zinc-800 ring-1 ring-white/10">
                    ...
                    <div className="flex flex-col leading-tight">
                        <span className="text-[13px] font-bold text-slate-200 font-sans tracking-wide">{tweet.user_name}</span>
                        <span className="text-[11px] text-slate-500 font-sans">@{tweet.user_screen_name}</span>
                    </div>
                </div>
            ...
            {/* Content Section */}
            <div className={`px-5 pb-6 ${hasMedia ? 'pt-2' : 'pt-2'}`}>
                {isQuoteStyle ? (
                    // Quote Card Style
                    <div className="relative p-6 rounded-xl bg-emerald-950/20 border border-emerald-500/10">
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500/50 rounded-l-xl" />
                        <p className="font-[family-name:var(--font-playfair)] italic text-[18px] leading-relaxed text-slate-200">
                            "{tweet.full_text}"
                        </p>
                    </div>
                ) : (
                    // Standard Text Style
                    <p className="font-serif text-[16px] leading-[1.625] text-slate-300 whitespace-pre-wrap">
                        {tweet.full_text}
                    </p>
                )}
            </div>
